...
people_client = PeopleClient()# 创建人民时报客户端实例
...
def generate_people_day_topic(model_type, model_name):
    config.llm_model_type = model_type
    ...
    markdown_file_path = people_client.export_people_article()
    report, report_file_path = report_generator.generate_people_article_report(markdown_file_path)
...
with gr.Blocks(title="GitHubSentinel") as demo:
    ...
    # 创建 Hacker News 热点话题 Tab
    with gr.Tab("人民日报 人民时评"):
        gr.Markdown("## 人民日报 人民时评")  # 添加小标题
        ...
        # 将按钮点击事件与导出函数绑定
        button.click(generate_people_day_topic, inputs=[model_type, model_name,], outputs=[markdown_output, file_output])    
