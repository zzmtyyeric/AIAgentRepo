...
class ReportGenerator:
    ...
    def generate_people_article_report(self, markdown_file_path):
        """
        生成 人民日报 每日主题的报告，并保存为 {original_filename}_topic.md。
        """
        with open(markdown_file_path, 'r',encoding="utf-8") as file:
            markdown_content = file.read()

        system_prompt = self.prompts.get("people_news_daily_article")
        report = self.llm.generate_report(system_prompt, markdown_content)
        
        report_file_path = os.path.splitext(markdown_file_path)[0] + "_comment.md"
        with open(report_file_path, 'w+') as report_file:
            report_file.write(report)

        LOG.info(f"人民日报 热点主题报告已保存到 {report_file_path}")
        return report, report_file_path
